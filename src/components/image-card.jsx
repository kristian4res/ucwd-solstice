...
const ImageCard = ({ imgUrl, imgTitle, sportType }) => {
    ...
    return (
            <div className="container flex justify-center rounded md:rounded-md m-0.5 overflow-hidden relative">
                <h4 className={`container absolute my-0 px-2 py-2 top-0 left-0 ${sportTypeColor[sportType]}`}>
                    <span className='text-xl text-white md:text-3xl'>{imgTitle}</span>
                </h4>
                <img id="image" 
                    src={imgUrl}
                    alt={'Winter sports'}
